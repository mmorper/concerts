...
**v6.0.0** 🎸 The Cure played the Hollywood Bowl in May 2023 and put five songs from *Songs of a Lost World* into the set — a record that was still seventeen months away. The archive has now worked out which album almost every song I've watched came off, so a setlist reads as a shape rather than a list: what the night leaned on, what was brand new, and what hadn't arrived yet. [Hear the arc](https://concerts.morperhaus.org/?scene=artists&artist=the-cure).
...
**New Capabilities**
Conversation has landed: the in-app ["Ask the Archive" chat](https://concerts.morperhaus.org/ask) lets you talk to the collection right on the site—ask a question in plain language and get answers grounded in real shows, rendered as little exhibits rather than walls of text (press ⌘K from anywhere). (The archive also speaks to AI assistants through its [MCP connector](https://concerts.morperhaus.org/about-mcp)—this brings that same voice onto the site itself.) Next: the Artist Scene gains a full discography view ([#5](https://github.com/mmorper/concerts/issues/5)), bringing every artist's catalog to life alongside the shows you saw—now that the archive knows which record each song came from, those two halves can finally meet. Audio preview playback comes to setlist items ([#22](https://github.com/mmorper/concerts/issues/22)), so you can hear any song from a night's setlist with a single click.
